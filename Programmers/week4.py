...
def solution(table, languages, preference):
    ...
    for lan in languages:
        for i in range(len(tmp_arr)):
            try:
                idx = tmp_arr[i].index(lan)
            except:
                temp.append(0)
                continue
            temp.append(len(tmp_arr[i]) - idx)
        cnt_arr.append(temp)
        temp = []

    weighted = np.array(list(map(list, zip(*cnt_arr))))
    matmul = weighted * preference
    result_sum = np.sum(matmul, axis=1)
    max_score = np.max(result_sum)
    
    idx_arr = np.where(result_sum == max_score)[0]
    for idx in idx_arr:
        result_arr.append(tmp_arr[idx][0])
    result_arr.sort()
